...
{
	if (!file.opendir(NowWorkPath.c_str()))
    {
    ...
    while (file.readdir())
    {
        if (!file.isHidden())
        {
            ...
            shell_print(file.getName());
            shell_print(file.isDir() ? "\tDIR" : "\tFILE");
            shell_printf("\t%d Bytes\r\n", file.isDir() ? 1234 : file.getSize());
        }
    }
...
#endif
	return f_mkdir(dir.c_str()) == FR_OK ? SHELL_RET_SUCCESS : SHELL_RET_FAILURE;
}
...
#endif
	return f_unlink(dir.c_str()) == FR_OK ? SHELL_RET_SUCCESS : SHELL_RET_FAILURE;
...
#endif
	return f_unlink(filePath.c_str()) == FR_OK ? SHELL_RET_SUCCESS : SHELL_RET_FAILURE;
}
