...
void TextEditor_OpenFile(const char* path)
{
    TextFileName = String(path);
        ...
        else if(obj == btnEdit)
        {
            ...
            if(IsEditting)
            {
                ...
                lv_obj_add_anim(
                    ...
                    lv_obj_get_height(ta_input), (lv_obj_get_height(appWindow) - lv_obj_get_height(topBar) - lv_obj_get_height(keyboard)),
                    500
                );
            }
            else
...
#endif
	        TextFile.seekSet(0);
	        TextFile.printf(TextBufferAddr);
...
static void KeyboardEvent_Handler(lv_obj_t * kb, lv_event_t event)
{
    ...
    if(event == LV_EVENT_CANCEL)
    {
        ...
        lv_obj_set_height(ta_input, (lv_obj_get_height(appWindow) - lv_obj_get_height(topBar)));
    }
...
static void Creat_TextArea(const char* text)
{
    ...
    lv_obj_set_size(ta_input, lv_obj_get_width(appWindow), lv_obj_get_height(appWindow) - lv_obj_get_height(topBar));
    lv_obj_align(ta_input, topBar, LV_ALIGN_OUT_BOTTOM_MID, 0, 0);
    lv_ta_set_text(ta_input, text ? text : "");
    lv_ta_set_placeholder_text(ta_input, "No text...");
    lv_ta_set_cursor_type(ta_input, LV_CURSOR_HIDDEN);
...
static void Setup()
{
    ...
    if(TextFile.isOpen())
    {
        ...
        TextFile.getName(nameBuff, sizeof(nameBuff));
        TextFileName = nameBuff;
    }
    else
    {
        TextFileName = "Editor";
    ...
    if(!IsEditingLocked)
    {
        Creat_TopBar(TextFileName.c_str());
