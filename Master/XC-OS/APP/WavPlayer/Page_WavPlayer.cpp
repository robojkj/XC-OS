...
static void Task_WavBarUpdate(lv_task_t * task)
{
	...
	//进度条数值更新
	int16_t ProgressValue = Music_get_Progress();
	//播放时间更新
	int sec = Music_get_cursec();
	
	lv_slider_set_value(barWav, ProgressValue, LV_ANIM_OFF);
	int min = sec / 60;
...
static void WavProgressEvent_Handler(lv_obj_t * obj, lv_event_t event)
{
    if(event == LV_EVENT_RELEASED)
    {
        uint16_t Progress = lv_slider_get_value(obj);
		//Serial.printf("Progress:%d\r\n", Progress);
		Music_setProgress(Progress);
		WavPlayer_VolumeRush();
    }
}
...
static void Creat_WavBar()
{
	...
	*/
	barWav = lv_slider_create(appWindow, NULL);
    //lv_obj_set_opa_scale_enable(sliderWavVolume true);
    lv_obj_set_size(barWav, APP_WIN_WIDTH - 100, 10);
    lv_obj_align(barWav, btnWav_Grp[BTN_STOP].btn, LV_ALIGN_OUT_TOP_MID, 0, -20);
    lv_obj_set_event_cb(barWav, WavProgressEvent_Handler);
    lv_slider_set_range(barWav, 0, 1000);
	lv_slider_set_anim_time(barWav, 1000);
	lv_slider_set_knob_size(barWav, 5);
	
	static lv_style_t style_knob;
	lv_style_copy(&style_knob, &lv_style_pretty);
	style_knob.body.radius = LV_RADIUS_CIRCLE;
	style_knob.body.opa = LV_OPA_30;
	
	lv_slider_set_style(barWav, LV_SLIDER_STYLE_KNOB, &style_knob);
...
static void ButtonEvent_Handler(lv_obj_t * obj, lv_event_t event)
{
    if(event == LV_EVENT_CLICKED)
    {
        ...
        if(obj == btnWav_Grp[BTN_NEXT].btn)
        {
            ...
            WavFilePath = MusicList_GetNowMusic();
            Wav_Update();
	        lv_label_set_text(btnWav_Grp[BTN_STOP].label, LV_SYMBOL_PAUSE);
        }
...
static void Setup()
{
    ...
    if(WavFilePath == "")
    {
        WavFilePath = MusicList_GetNextMusic(false);
    }
