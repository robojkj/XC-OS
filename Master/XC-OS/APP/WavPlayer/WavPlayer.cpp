...
bool WavPlayer_LoadFile(String path)
{
	...
	*/
	...
	if(decode_Mode == DECODE_WAV)
	{
		...
		_file_buffer = (uint8_t*)MemPool_Malloc(WAVFIL_BUFFER_SIZE);
	...
	{
		_file_buffer = (uint8_t*)MemPool_Malloc(MP3INPUTBUF_SIZE);
		if(_file_buffer == NULL)
		{
	...
	//初始化DAC设备
	DAC1_Base = (DAC_Base_TypeDef*)MemPool_Malloc(sizeof(DAC_Base_TypeDef));
	if(DAC1_Base == NULL)
	{
		Audio_SetEnable(false);
	...
	DAC2_Base = (DAC_Base_TypeDef*)MemPool_Malloc(sizeof(DAC_Base_TypeDef));
	if(DAC2_Base == NULL)
	{
		Audio_SetEnable(false);
	...
	DAC1_Base->buffer1 = (uint16*)MemPool_Malloc(_buffer_size *2);
	DAC1_Base->buffer2 = (uint16*)MemPool_Malloc(_buffer_size *2);
	DAC2_Base->buffer1 = (uint16*)MemPool_Malloc(_buffer_size *2);
	DAC2_Base->buffer2 = (uint16*)MemPool_Malloc(_buffer_size *2);
...
void Task_WavPlayer(void *pvParameters)
{
    ...
    for(;;)
    {
		...
		if(Music_getdecode_Mode() == DECODE_MP3)
			mp3_deinit();
