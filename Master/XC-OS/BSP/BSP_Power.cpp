...
void Task_BattInfoUpdate(TimerHandle_t xTimer)
{
    __ExecuteOnce((
		...
		pinMode(PWR_DEV_Pin, OUTPUT),
		pinMode(PWR_KEY_Pin, OUTPUT),
		digitalWrite(PWR_KEY_Pin, LOW),
		digitalWrite(PWR_DEV_Pin, LOW),
		delay(100),
		digitalWrite(PWR_DEV_Pin, HIGH)
    ));
    ...
    //Serial.printf("Oc U = %0.2fmV, IRQ = %d\r\n", charger.getBattOcVoltage(), digitalRead(CHG_KEY_Pin));
}
