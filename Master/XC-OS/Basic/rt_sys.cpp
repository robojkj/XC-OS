...
FILEHANDLE _sys_open(const char *name, int openmode)
{
	...
	if (openmode & OPEN_R) flags |= FA_READ;
	if (openmode & OPEN_W) flags |= (FA_WRITE | FA_OPEN_ALWAYS);
	//if (openmode & OPEN_A) flags |= O_APPEND;
    if (openmode & OPEN_B) flags |= FA_READ | FA_WRITE | FA_OPEN_ALWAYS;
	...
	//static void* file_p;
    file_t* file_p = new file_t;
	FRESULT res = f_open((FIL*)file_p, name, flags);
               ...
               unsigned len, int mode)
{
	...
	FRESULT res = f_write((FIL*)file_p, buf, len, NULL);
              ...
              unsigned len, int mode)
{
	...
	FRESULT res = f_read((FIL*)file_p, buf, len, NULL);
	if (res == FR_OK) return 0;
	else return -1;
}
...
#endif
	return f_lseek((FIL*)file_p, pos) == true ? 0 : -1;
}
