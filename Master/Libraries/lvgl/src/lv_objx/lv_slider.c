...
static bool lv_slider_design(lv_obj_t * slider, const lv_area_t * mask, lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
		...
		knob_area.x1 -= ext->knob_size;
        knob_area.x2 += ext->knob_size;
        knob_area.y1 -= ext->knob_size;
        knob_area.y2 += ext->knob_size;
...
void lv_slider_set_knob_size(const lv_obj_t * slider, lv_coord_t size)
{
    LV_ASSERT_OBJ(slider, LV_OBJX_NAME);
    lv_slider_ext_t * ext = lv_obj_get_ext_attr(slider);
...
lv_coord_t lv_slider_get_knob_size(const lv_obj_t * slider)
{
    LV_ASSERT_OBJ(slider, LV_OBJX_NAME);
    lv_slider_ext_t * ext = lv_obj_get_ext_attr(slider);

    return ext->knob_size;
}
